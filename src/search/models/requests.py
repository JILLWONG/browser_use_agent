...
class BaseSearchRequest(BaseModel):
    ...
    num_results: int = Field(
        default=5, description="Number of results to return", ge=1, le=10
    )
    ...
    max_len: int | None = Field(
        default=None, description="Maximum content length", ge=1
    )
...
class SingleSearchRequest(BaseSearchRequest):
    ...
    fetch_content: bool = Field(
        default=True, description="Whether to fetch web content"
    )
...
class BatchSearchRequest(BaseModel):
    ...
    searches: list[BatchSearchItem] = Field(
        ..., description="List of search items", min_items=1
    )
    fetch_content: bool = Field(
        default=True, description="Whether to fetch web content"
    )
...
class AgenticSearchRequest(BaseModel):
    ...
    llm_model_name: str = Field(
        default="qwen/qwen-plus", description="OpenRouter LLM model name", min_length=1
    )

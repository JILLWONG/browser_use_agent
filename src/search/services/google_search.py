...
class GoogleSearchService:
    ...
    def search(
        ...
        if not api_key or not cse_id:
            self.logger.error(
                f"[{request_id}] API key or CSE ID not provided for query: {query}"
            )
        ...
        try:
            self.logger.info(
                f"[{request_id}] Executing Google search for query: '{query}'"
            )
            response = requests.get(
                self.service_url, params=params, timeout=self.settings.request_timeout
            )
            ...
            if "items" in data:
                for item in data["items"]:
                    results.append(
                        SearchResult(
                            url=item.get("link", ""), abstract=item.get("snippet", "")
                        )
                    )
            ...
            search_time = time.time() - search_start
            self.logger.error(
                f"[{request_id}] Request error during Google search for '{query}': {e} - Time: {search_time:.3f}s"
            )
            self.metrics.increment("failed_searches")
            return [], False

...
class SearchOrchestratorService:
    ...
    def process_single_search_request(
        self, request: SingleSearchRequest, request_id: str
    ) -> SingleSearchResponse:
        ...
        if not search_success:
            self.logger.error(
                f"[{request_id}] Search failed for query: '{request.query}'"
            )
    ...
    def process_batch_search_request(
        self, request: BatchSearchRequest, request_id: str
    ) -> BatchSearchResponse:
        ...
        def process_batch_search_item(search_item, index: int):
            ...
            try:
                ...
                self.logger.error(
                    f"[{request_id}-{index}] Error processing batch item: {e}"
                )
                return {
                    "query": search_item.query,
                    "success": False,
        ...
        with ThreadPoolExecutor(
            max_workers=min(self.settings.max_workers, len(request.searches))
        ) as executor:
            future_to_index = {
            ...
            for future in as_completed(future_to_index):
                try:
                    ...
                    self.logger.error(
                        f"[{request_id}] Error in batch search {index}: {e}"
                    )
                    results.append(
                        {
    ...
    def _process_queries_concurrent(
        ...
        def process_single_query(query: str) -> tuple[str, list[SearchResult]]:
            ...
            if not search_success or not search_results:
                self.logger.warning(
                    f"[{request_id}] No search results for query: {query}"
                )
                return query, []
        ...
        with ThreadPoolExecutor(
            max_workers=min(self.settings.max_workers, len(queries))
        ) as executor:
            future_to_query = {
            ...
            for future in as_completed(future_to_query):
                try:
                    ...
                    self.logger.error(
                        f"[{request_id}] Error processing query '{query}': {e}"
                    )
                    output_data[query] = []
    ...
    def _fetch_content_for_results(
        ...
        def fetch_single_result(result: SearchResult) -> SearchResult:
            ...
            if result.url:
                content, fetch_success, is_truncated = (
        ...
        enhanced_results = []
        with ThreadPoolExecutor(
            max_workers=min(self.settings.max_content_workers, len(search_results))
        ) as executor:
            future_to_result = {
            ...
            for future in as_completed(future_to_result):
                try:
                    ...
                    self.logger.error(
                        f"[{request_id}] Error fetching content for result: {e}"
                    )

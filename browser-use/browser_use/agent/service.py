...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		save_trace_with_img_path: str | None = None,
		max_failures: int = 3,
		retry_delay: int = 10,
		...
		self.settings = AgentSettings(
			...
			save_trace_with_img_path=save_trace_with_img_path,
	...
	async def step(self, step_info: AgentStepInfo | None = None) -> None:
		...
		try:
			...
			# 将上一个step的工具执行结果action results加入到message里面（历史部分），并将当前页面的可点击区域和截图加入到message里面
			...
			try:
				...
				# 将最后一条message（页面可点击列表+截图）去掉
				self._message_manager._remove_last_state_message()  # we dont want the whole state in the chat history

				# check again if Ctrl+C was pressed before we commit the output to history
				await self._raise_if_stopped_or_paused()

				# 将当前LLM的输出action（tool call）加入到message里面（历史部分）
			...
			if state:
				...
				self._make_history_item(model_output, state, result, metadata, input_messages)
	...
	def _make_history_item(
		...
		if input_messages and len(input_messages)>0:
			for msg in reversed(input_messages):
				if isinstance(msg, HumanMessage) and isinstance(msg.content,list) and isinstance(msg.content[0], dict):
					snapshot = msg.content[0].get("text").split("if you need to remember it write it to memory:")[-1].split("Current step:")[0].strip()
					break
		...
		state_history = BrowserStateHistory(
			...
			screenshot_no_highlights=state.screenshot_no_highlights,
		)

		history_item = AgentHistory(snapshot=snapshot,model_output=model_output, result=result, state=state_history, metadata=metadata)
